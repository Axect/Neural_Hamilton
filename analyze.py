...
def calculate_comparison_results(model, device, dl_true, dl_y4, dl_rk4, variational=False):
    """
...
def plot_comparison_histograms(results_dict, fig_dir):
    ...
    loss_min_log = np.log10(all_losses.min()) * 1.01 if all_losses.min() < 0 else np.log10(all_losses.min()) * 0.99
    loss_max_log = np.log10(all_losses.max()) * 0.99 if all_losses.max() < 0 else np.log10(all_losses.max()) * 1.01
    ...
    with plt.style.context(["science", "nature"]):
        ...
        ax.legend()
        fig.savefig(f"{fig_dir}/00_4_Loss_comparison_hist.png", dpi=600, bbox_inches="tight")
        plt.close(fig)
...
def plot_detailed_comparisons(results_dict, V, t, q_true, p_true, fig_dir, indices=None):
    ...
    for idx in indices:
        if idx >= num_potentials:
            print(f"Warning: Index {idx} exceeds available potentials ({num_potentials})")
            continue
        ...
        with plt.style.context(["science", "nature"]):
            ...
            ax.set_ylabel(r"$V(q)$")
            ax.autoscale(tight=True)
            fig.savefig(f"{fig_dir}/{idx:02}_potential.png", dpi=600, bbox_inches="tight")
            plt.close(fig)
            ...
            fig.savefig(f"{fig_dir}/{idx:02}_q_comparison.png", dpi=600, bbox_inches="tight")
            plt.close(fig)
            ...
            fig.savefig(f"{fig_dir}/{idx:02}_p_comparison.png", dpi=600, bbox_inches="tight")
            plt.close(fig)
            ...
            fig.savefig(f"{fig_dir}/{idx:02}_phase_comparison.png", dpi=600, bbox_inches="tight")
            plt.close(fig)


def plot_kl8_model_only(results_dict, V, t, q_true, p_true, fig_dir, indices=None):
    """
    ...
    for idx in indices:
        ...
        with plt.style.context(["science", "nature"]):
            ...
            fig.savefig(f"{fig_dir}/{idx:02}_kl8_potential.png", dpi=600, bbox_inches="tight")
            plt.close(fig)
            ...
            ax.legend()
            fig.savefig(f"{fig_dir}/{idx:02}_kl8_q_only.png", dpi=600, bbox_inches="tight")
            plt.close(fig)
            ...
            ax.legend()
            fig.savefig(f"{fig_dir}/{idx:02}_kl8_p_only.png", dpi=600, bbox_inches="tight")
            plt.close(fig)
            ...
            ax.legend()
            fig.savefig(f"{fig_dir}/{idx:02}_kl8_phase_only.png", dpi=600, bbox_inches="tight")
            plt.close(fig)
...
def main():
    ...
    if test_option == "test":
        ...
        if test_mode == "Original (Y4 Reference)":
            ...
            console.print("[bold yellow]Loading KahanLi8 reference data...[/bold yellow]")
            ...
            try:
                ...
                console.print("[bold yellow]KahanLi8 reference data not found. Run the Julia script test_reference.jl first.[/bold yellow]")
