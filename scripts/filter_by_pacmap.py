...
def pacmap_map_and_embed(
    ...
    # Performs PaCMAP dimensionality reduction using default parameters.
    ...
    # df_no_id should not contain 'index' or 'Cluster' before PaCMAP
    ...
    if len(data_for_pacmap) == 0:
        ...
        if cluster_series is not None:
            empty_pacmap_df["Cluster"] = pd.Series(dtype=cluster_series.dtype)

        # Create an empty mapper instance if data is empty
        ...
        # For empty data, we can't fit_transform, so just return empty results
        return empty_pacmap_df, mapper

    # Using PaCMAP with default parameters
    mapper = pacmap.PaCMAP(
        n_components=2,
        random_state=random_state,
    )
    ...
    pacmap_df["index"] = index_series.values # Restore original index
    if cluster_series is not None: # If input df had clusters, add them back
        # This is important if we are PaCMAPing an already sampled and clustered DF
        if len(cluster_series) == len(pacmap_df):
            pacmap_df["Cluster"] = cluster_series.values
        else:
            print(f"Warning: Length mismatch between PaCMAP data ({len(pacmap_df)}) and original cluster_series ({len(cluster_series)}). Clusters not added to PaCMAP output.")
...
def kmeans_clustering(
    ...
    # Performs KMeans clustering on PaCMAP coordinates.
    # Expects 'df' to have 'PaCMAP1', 'PaCMAP2', and 'index' columns.
    index_series = df['index'].copy()

    if not ({'PaCMAP1', 'PaCMAP2'}.issubset(df.columns)):
        print("Warning: PaCMAP1 or PaCMAP2 not found in DataFrame for KMeans. Returning empty cluster DataFrame.")
        return pd.DataFrame( {"Cluster": pd.Series(dtype=np.int32), "index": pd.Series(dtype=index_series.dtype)})

    df_for_kmeans = df[["PaCMAP1", "PaCMAP2"]]
...
def plot_pacmap_data(
    pacmap_coords_df: pd.DataFrame, # Should contain 'PaCMAP1', 'PaCMAP2'
    cluster_labels_series: pd.Series, # Series of cluster labels, matching pacmap_coords_df
    title: str,
    filename: str,
):
    # Plots PaCMAP 2D projection with optional cluster coloring.
    ...
    with plt.style.context(["science", "nature"]): # Using specified style context
        ...
        if not pacmap_coords_df.empty and "PaCMAP1" in pacmap_coords_df and "PaCMAP2" in pacmap_coords_df:
            # Ensure cluster_labels_series is not empty and has the same length as pacmap_coords_df
            if not cluster_labels_series.empty and len(cluster_labels_series) == len(pacmap_coords_df) and cluster_labels_series.notna().all():
                color_data = cluster_labels_series.to_numpy().astype(int)
                scatter = ax.scatter(
                    pacmap_coords_df["PaCMAP1"].to_numpy(),
                    pacmap_coords_df["PaCMAP2"].to_numpy(),
                    c=color_data,
                ...
                if cluster_labels_series.empty:
                    print(f"Info: Cluster labels are empty for PaCMAP plot '{title}'. Plotting without colors.")
                elif len(cluster_labels_series) != len(pacmap_coords_df):
                    print(f"Warning: Mismatch in length between PaCMAP data and cluster labels for '{title}'. Plotting without colors.")
                elif cluster_labels_series.isna().any():
                    print(f"Warning: Cluster labels contain NaN values for PaCMAP plot '{title}'. Plotting without colors.")

        else:
            print(f"Warning: PaCMAP coordinate data is empty or invalid for plotting '{title}'. Skipping scatter plot.")
        ...
        try:
            ...
            print(f"Saved PaCMAP plot: {filename}")
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PaCMAP and KMeans clustering with inverse point count weighted sampling.")
    parser.add_argument(
        "--data",
        type=str,
        required=True,
    ...
    if args.data == "test":
        ...
        # PaCMAP embedding on full reshaped data
        pacmap_active, _ = pacmap_map_and_embed(
        ...
        # KMeans clustering on PaCMAP results
        # pacmap_active should contain 'index', 'PaCMAP1', 'PaCMAP2'
        cluster_active = kmeans_clustering(pacmap_active, n_clusters=n_clusters_active, random_state=42)

        # Plot PaCMAP of full test data with cluster colors
        if not pacmap_active.empty and not cluster_active.empty:
            # Merge cluster_active with pacmap_active for plotting
            # Need to align cluster labels with pacmap_active based on 'index'
            plot_data_full_active = pacmap_active.merge(cluster_active, on='index', how='left')
            if 'Cluster' in plot_data_full_active.columns:
                plot_pacmap_data(
                    pacmap_coords_df=plot_data_full_active[["PaCMAP1", "PaCMAP2"]],
                    cluster_labels_series=plot_data_full_active["Cluster"],
                    title=f"PaCMAP projection of test data ({args.data}, k={n_clusters_active})",
                    filename=f"figs/pacmap_projection_{args.data}_full.png",
                )
            else:
                print("Warning: 'Cluster' column not found after merging for full active data plot.")
        else:
            print("Skipping PaCMAP plot for full test data: PaCMAP or cluster data is empty.")
        ...
        # Prepare data for sampling: merge reshaped data with PaCMAP and cluster info
        # df_active_reshaped has 'index' and features (V0, V1, ...)
        # pacmap_active has 'index', 'PaCMAP1', 'PaCMAP2'
        # cluster_active has 'index', 'Cluster'
        df_active_for_sampling = df_active_reshaped.merge(pacmap_active, on="index", how="left")
        ...
        # df_active_for_sampling must contain 'index', 'Cluster', (and optionally features, PaCMAP coords for consistency)
        ...
        # Plot PaCMAP of sampled test data
        if not sampled_active.empty:
            # sampled_active now contains original features, PaCMAP1, PaCMAP2, Cluster, index
            # We can directly use its PaCMAP coordinates for plotting, no need to re-embed if they are there.
            # However, for consistency with train/val and if PaCMAP was dropped, let's re-embed the sampled features.
            # The 'Cluster' column in sampled_active is the original cluster ID from full data.
            
            # Option 2: Use existing PaCMAP coordinates if they were carried into sampled_active
            if 'PaCMAP1' in sampled_active.columns and 'PaCMAP2' in sampled_active.columns and 'Cluster' in sampled_active.columns:
                 plot_pacmap_data(
                     pacmap_coords_df=sampled_active[["PaCMAP1", "PaCMAP2"]],
                     cluster_labels_series=sampled_active["Cluster"],
                     title=f"PaCMAP of sampled test data ({args.data}, k={n_clusters_active})",
                     filename=f"figs/pacmap_projection_sampled_{args.data}.png",
                 )
            else: # Fallback to re-embedding if PaCMAP coords are not directly in sampled_active
                print("Info: PaCMAP1/PaCMAP2 not directly in sampled_active, attempting re-embedding for plot.")
                # Drop potentially existing PaCMAP/Cluster to avoid issues in pacmap_map_and_embed if it expects raw features
                df_to_pacmap_sampled = sampled_active.drop(columns=['PaCMAP1', 'PaCMAP2', 'Cluster'], errors='ignore')
                
                # PaCMAP with default parameters will handle small sample sizes
                pacmap_sampled_active_coords, _ = pacmap_map_and_embed(
                     df_to_pacmap_sampled, # Should contain 'index' and features
                     random_state=42
                )
                # Merge with the original cluster labels from sampled_active
                if 'index' in pacmap_sampled_active_coords.columns and 'index' in sampled_active.columns and 'Cluster' in sampled_active.columns:
                     pacmap_sampled_active_plot_df = pacmap_sampled_active_coords.merge(sampled_active[['index', 'Cluster']], on='index', how='left')
                     if 'Cluster' in pacmap_sampled_active_plot_df.columns and not pacmap_sampled_active_plot_df.empty:
                         plot_pacmap_data(
                             pacmap_coords_df=pacmap_sampled_active_plot_df[["PaCMAP1", "PaCMAP2"]],
                             cluster_labels_series=pacmap_sampled_active_plot_df["Cluster"],
                             title=f"PaCMAP of sampled test data ({args.data}, k={n_clusters_active})",
                             filename=f"figs/pacmap_projection_sampled_{args.data}.png",
                         )
                     else:
                         print("Warning: Could not prepare PaCMAP plot for sampled active data (missing 'Cluster' after merge or empty).")
                else:
                     print("Warning: 'index' or 'Cluster' missing for sampled active PaCMAP plot preparation.")


        else:
            print("Skipping PaCMAP plot for sampled test data: sampled_active is empty.")
        ...
        if df_train_original.empty:
            ...
            if df_train_reshaped.empty:
                ...
                pacmap_train, _ = pacmap_map_and_embed(
                    df_train_reshaped.drop(columns=['Cluster'], errors='ignore'),
                    random_state=42
                )
                cluster_train = kmeans_clustering(pacmap_train, n_clusters=n_clusters_train, random_state=42)

                if not pacmap_train.empty and not cluster_train.empty:
                    plot_data_full_train = pacmap_train.merge(cluster_train, on='index', how='left')
                    if 'Cluster' in plot_data_full_train.columns:
                        plot_pacmap_data(
                            pacmap_coords_df=plot_data_full_train[["PaCMAP1", "PaCMAP2"]],
                            cluster_labels_series=plot_data_full_train["Cluster"],
                            title=f"PaCMAP projection of training data ({args.data}, k={n_clusters_train})",
                            filename=f"figs/pacmap_projection_{args.data}_train_full.png",
                        )
                else:
                    print("Skipping PaCMAP plot for full training data: PaCMAP or cluster data is empty.")
                ...
                df_train_for_sampling = df_train_reshaped.merge(pacmap_train, on="index", how="left")
                ...
                if not sampled_train.empty:
                    if 'PaCMAP1' in sampled_train.columns and 'PaCMAP2' in sampled_train.columns and 'Cluster' in sampled_train.columns:
                        plot_pacmap_data(
                            pacmap_coords_df=sampled_train[["PaCMAP1", "PaCMAP2"]],
                            cluster_labels_series=sampled_train["Cluster"],
                            title=f"PaCMAP of sampled training data ({args.data}, k={n_clusters_train})",
                            filename=f"figs/pacmap_projection_sampled_{args.data}_train.png",
                        )
                    else: # Fallback to re-embedding
                        print("Info: PaCMAP1/PaCMAP2 not directly in sampled_train, attempting re-embedding for plot.")
                        df_to_pacmap_sampled_train = sampled_train.drop(columns=['PaCMAP1', 'PaCMAP2', 'Cluster'], errors='ignore')
                        pacmap_sampled_train_coords, _ = pacmap_map_and_embed(
                            df_to_pacmap_sampled_train,
                            random_state=42)
                        if 'index' in pacmap_sampled_train_coords.columns and 'index' in sampled_train.columns and 'Cluster' in sampled_train.columns:
                            pacmap_sampled_train_plot_df = pacmap_sampled_train_coords.merge(sampled_train[['index', 'Cluster']], on='index', how='left')
                            if 'Cluster' in pacmap_sampled_train_plot_df.columns and not pacmap_sampled_train_plot_df.empty:
                                plot_pacmap_data(
                                    pacmap_coords_df=pacmap_sampled_train_plot_df[["PaCMAP1", "PaCMAP2"]],
                                    cluster_labels_series=pacmap_sampled_train_plot_df["Cluster"],
                                    title=f"PaCMAP of sampled training data ({args.data}, k={n_clusters_train})",
                                    filename=f"figs/pacmap_projection_sampled_{args.data}_train.png",
                                )
                else:
                    print("Skipping PaCMAP plot for sampled training data: sampled_train is empty.")
        ...
        if df_val_original.empty:
            ...
            if df_val_reshaped.empty:
                ...
                pacmap_val, _ = pacmap_map_and_embed(
                    df_val_reshaped.drop(columns=['Cluster'], errors='ignore'),
                    random_state=42
                )
                cluster_val = kmeans_clustering(pacmap_val, n_clusters=n_clusters_val, random_state=42)
                
                if not pacmap_val.empty and not cluster_val.empty:
                    plot_data_full_val = pacmap_val.merge(cluster_val, on='index', how='left')
                    if 'Cluster' in plot_data_full_val.columns:
                        plot_pacmap_data(
                            pacmap_coords_df=plot_data_full_val[["PaCMAP1", "PaCMAP2"]], 
                            cluster_labels_series=plot_data_full_val["Cluster"], 
                            title=f"PaCMAP of validation data ({args.data}, k={n_clusters_val})", 
                            filename=f"figs/pacmap_projection_{args.data}_val_full.png"
                        )
                else:
                    print("Skipping PaCMAP plot for full validation data: PaCMAP or cluster data is empty.")
                ...
                df_val_for_sampling = df_val_reshaped.merge(pacmap_val, on="index", how="left")
                ...
                if not sampled_val.empty:
                    if 'PaCMAP1' in sampled_val.columns and 'PaCMAP2' in sampled_val.columns and 'Cluster' in sampled_val.columns:
                        plot_pacmap_data(
                            pacmap_coords_df=sampled_val[["PaCMAP1", "PaCMAP2"]],
                            cluster_labels_series=sampled_val["Cluster"],
                            title=f"PaCMAP of sampled validation data ({args.data}, k={n_clusters_val})",
                            filename=f"figs/pacmap_projection_sampled_{args.data}_val.png",
                        )
                    else: # Fallback to re-embedding
                        print("Info: PaCMAP1/PaCMAP2 not directly in sampled_val, attempting re-embedding for plot.")
                        df_to_pacmap_sampled_val = sampled_val.drop(columns=['PaCMAP1', 'PaCMAP2', 'Cluster'], errors='ignore')
                        pacmap_sampled_val_coords, _ = pacmap_map_and_embed(
                            df_to_pacmap_sampled_val,
                            random_state=42)
                        if 'index' in pacmap_sampled_val_coords.columns and 'index' in sampled_val.columns and 'Cluster' in sampled_val.columns:
                            pacmap_sampled_val_plot_df = pacmap_sampled_val_coords.merge(sampled_val[['index', 'Cluster']], on='index', how='left')
                            if 'Cluster' in pacmap_sampled_val_plot_df.columns and not pacmap_sampled_val_plot_df.empty:
                                plot_pacmap_data(
                                    pacmap_coords_df=pacmap_sampled_val_plot_df[["PaCMAP1", "PaCMAP2"]],
                                    cluster_labels_series=pacmap_sampled_val_plot_df["Cluster"],
                                    title=f"PaCMAP of sampled validation data ({args.data}, k={n_clusters_val})",
                                    filename=f"figs/pacmap_projection_sampled_{args.data}_val.png",
                                )
                else:
                    print("Skipping PaCMAP plot for sampled validation data: sampled_val is empty.")
