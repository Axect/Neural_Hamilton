...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let path = format!("{}{}", path, "morse.parquet");
    ...
    let path = format!("{}{}", path, "morse_rk4.parquet");
    df.write_parquet(&path, CompressionOptions::Uncompressed)?;
...
#[allow(non_snake_case)]
fn V(x: f64) -> f64 {
    let a = 3f64 * ((1f64 + 5f64.sqrt()) / 2f64).ln();
    let D_e = 8f64 / (5f64.sqrt() - 1f64).powi(2);
    let r_e = 1f64 / 3f64;
    D_e * (1f64 - (-a * (x - r_e)).exp()).powi(2)
}
...
fn dVdx(x: f64) -> f64 {
    ...
    let D_e = 8f64 / (5f64.sqrt() - 1f64).powi(2);
    let r_e = 1f64 / 3f64;
    -2f64 * D_e * (1f64 - (-a * (x - r_e)).exp()) * a * (-a * (x - r_e)).exp()
}

struct Morse;

impl ODEProblem for Morse {
